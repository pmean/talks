...
## Discontinuous cubic polynomials
...
spline_columns <- names(xm)[c(2:4, 8, 12, 16)]
xm |>
  select(all_of(spline_columns)) |>
  cor() |>
  data.frame() |>
  round(2) -> spline_correlations

bind_cols(data.frame(`-`=spline_columns), spline_correlations) |>
  gt()
```
